...
export default function PaymentHistoryPage() {
  ...
  const handleDownloadCSV = () => {
    try {
      const csvData = metrics.recentPayments.map((loan) => ({
      ...
      exportToCSV(csvData, 'payment-history.csv');
    } catch (err) {
  ...
  if (error) {
    return <ErrorState message="Failed to fetch payment history data. Please ensure the backend server is running." onRetry={() => refetch()} />;
  }
    ...
    <div className="p-8">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              ₹{metrics.totalInterestPaid.toLocaleString('en-IN')}
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              ₹{metrics.totalPrincipalPaid.toLocaleString('en-IN')}
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{metrics.loansWithPayments.length}</div>
            <p className="text-xs text-muted-foreground">
              {((metrics.loansWithPayments.length / loans.length) * 100).toFixed(1)}% of total
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              ₹{metrics.avgPaymentPerLoan.toLocaleString('en-IN', { maximumFractionDigits: 0 })}
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <Table>
              ...
              <TableBody>
                {metrics.recentPayments.map((loan) => {
